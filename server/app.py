...
# Ensure the project root is on sys.path so models.py can be found
...
def get_supabase() -> Client:
    ...
    if _supabase_client is None:
        ...
        if not url or not key:
            raise HTTPException(status_code=503, detail="Database not configured: SUPABASE_URL and SUPABASE_KEY are required")
...
async def login(request: LoginRequest):
    ...
    try:
        ...
        res = get_supabase().table("profiles").select("*").eq("player_name", request.player_name).execute()
        ...
        if res.data:
            ...
            if request.avatar_url:
                get_supabase().table("profiles").update({"avatar_url": request.avatar_url}).eq("id", profile["id"]).execute()
            ...
            res = get_supabase().table("profiles").insert(new_profile).execute()
            profile = res.data[0]
...
async def get_leaderboard():
    ...
    try:
        res = get_supabase().table("profiles").select("player_name, score, coins, world").order("score", desc=True).limit(10).execute()
        return res.data
    except Exception as e:
...
async def get_hint():
    ...
    try:
        ...
        try:
            ...
            get_supabase().table("profiles").update({"coins": new_coins}).eq("id", env_state.current_profile["id"]).execute()
...
async def buy_powerup(powerup_id: str = Query(...)):
    ...
    if powerup_id == "heart_restore":
        ...
        try:
            get_supabase().table("profiles").update({"hearts": new_hearts, "coins": new_coins}).eq("id", p["id"]).execute()
        ...
        new_coins = p.get("coins", 0) - cost
        try:
            get_supabase().table("profiles").update({"coins": new_coins}).eq("id", p["id"]).execute()
            env_state.current_profile["coins"] = new_coins
...
async def get_stats():
    ...
    try:
        ...
        res = get_supabase().table("triage_logs").select("*").eq("player_id", pid).order("created_at", desc=True).limit(10).execute()
        logs = res.data
        
        avg_score = sum(log["score"] for log in logs) / len(logs) if logs else 0
        total_res = get_supabase().table("triage_logs").select("id", count="exact").eq("player_id", pid).execute()
...
async def step(action: Action):
    ...
    if env_state.current_profile:
        try:
            ...
            get_supabase().table("triage_logs").insert(audit_log).execute()
