...
ULTRA-FIXED VERSION - Guaranteed healthcheck response
"""
...
def wait_for_server_ready(port, timeout=10):
    """Wait until the server is actually accepting connections."""
    start_time = time.time()
    while time.time() - start_time < timeout:
        try:
            with socket.create_connection(('127.0.0.1', port), timeout=1):
                log(f"[SERVER] Port {port} is ready and accepting connections")
                return True
...
def run_server():
    """
    ULTRA-FIXED: Start server synchronously, verify it's ready, then run tasks.
    """
    ...
    try:
        ...
        httpd.allow_reuse_address = True  # CRITICAL: Allow port reuse
        ...
        # CRITICAL: Wait for server to actually be ready
        if not wait_for_server_ready(HEALTHCHECK_PORT, timeout=10):
            log("[SERVER] WARNING: Server may not be ready, but continuing...")
        
        log("[SERVER] Healthcheck server is running and ready")
